...
@section('content')
    <x-core::card>
        <x-core::card.header>
            <x-core::card.title>Sort for Products Page</x-core::card.title>
            <div class="d-flex gap-2">
                <x-core::button tag="a" :href="route('ecommerce.product-sort.products.index')" color="primary">Products Page</x-core::button>
                <x-core::button tag="a" :href="route('ecommerce.product-sort.categories.index')">Category Pages</x-core::button>
            </div>
        </x-core::card.header>
        <x-core::card.body>
            @if (session('status'))<x-core::alert type="success" :dismissible="true">{{ session('status') }}</x-core::alert>@endif
            <form method="post" action="{{ route('ecommerce.product-sort.products.update') }}" id="sort-products-form">
                @csrf
                <p class="text-muted">Drag and drop cards. Top = highest priority. Values auto-updated.</p>
                <div class="row g-3" id="sortable-products">
                        ...
                        <div class="col-12 col-sm-6 col-lg-4 col-xl-3" draggable="true" data-product-id="{{ $product->id }}">
                            <div class="card h-100">
                                <div class="card-body">
                                    ...
                                    <label class="form-label mt-2 mb-1">Sort (Products)</label>
                                    <input type="number" min="0" name="sort_order_product_page[{{ $product->id }}]" value="{{ (int) $product->sort_order_product_page }}" class="form-control sort-input">
                                </div>
                            </div>
                        </div>
                    @endforeach
